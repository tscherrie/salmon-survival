...
// A larger characin carries a proportionally smaller, flatter eye. Presses the eyeball
// and the orbit that follows its shape back toward the head by `amount` of the bulge, so
// eye and skin stay joined.
// `shrink` draws the whole orbit in toward its centre, to that share of its radius,
// stretching the skin around it to meet the head as before.
export const EYE_SHRINK = 0.62;
export function flattenEyes(geometry, amount, shrink = EYE_SHRINK) {
  ...
  for (let i = 0; i < position.count; i++) {
    ...
    const dx = x - EYE.x,
      dy = y - EYE.y;
    const orbit = Math.hypot(dx / EYE.radiusX, dy / EYE.radiusY);
    if (orbit > reach || Math.abs(z) < 1e-5) continue;
    const dome = EYE.bulge * Math.sqrt(Math.max(0, 1 - Math.min(1, orbit) ** 2));
    const fade = 1 - THREE.MathUtils.smoothstep(orbit, 0.95, 1.25);
    position.setZ(i, z - Math.sign(z) * dome * amount * fade);
    const drawn =
      orbit < 1 ? orbit * shrink : shrink + ((orbit - 1) * (reach - shrink)) / (reach - 1);
    const k = orbit > 1e-6 ? drawn / orbit : shrink;
    position.setX(i, EYE.x + dx * k);
    position.setY(i, EYE.y + dy * k);
  }
  ...
  shader.fragmentShader = shader.fragmentShader
    ...
    .replace(
      ...
      if (vFishPart < 0.5) {
        ...
        #ifdef FISH_BRYCON
          // Piraputanga, Brycon hilarii: a big bright-silver characin with a bronze-olive
          // back, a white belly, a dark humeral mark behind the gill cover and a black
          // blotch on the caudal peduncle that runs out along the middle of the tail.
          vec3 brycon = mix(vec3(0.02, 0.024, 0.014), vec3(0.09, 0.085, 0.05),
            smoothstep(0.05, 0.34, fishBand));
          brycon = mix(brycon, vec3(0.58, 0.59, 0.55), smoothstep(0.32, 0.52, fishBand));
          brycon = mix(brycon, vec3(0.80, 0.79, 0.74), smoothstep(0.62, 0.92, fishBand));
          vec2 bryconGrid = fishScaleGrid();
          float bryconRim = smoothstep(0.34, 0.5, length((fract(bryconGrid) - 0.5) * vec2(0.85, 1.0)));
          brycon *= 1.0 + (fishHash(floor(bryconGrid)) - 0.5) * 0.12 * fishScaleMask()
            - bryconRim * 0.09 * fishScaleMask();
          float humeral = exp(-pow((fishX - 0.165) / 0.022, 2.0) - pow((fishBand - 0.33) / 0.09, 2.0));
          float peduncle = (1.0 - smoothstep(-0.29, -0.2, fishX)) * exp(-pow((fishBand - 0.5) / 0.16, 2.0));
          brycon = mix(brycon, vec3(0.02, 0.022, 0.02), max(humeral * 0.75, peduncle * 0.85));
          // A warm, golden cast to the upper flank and the head.
          brycon = mix(brycon, brycon * vec3(1.18, 1.05, 0.7), smoothstep(0.1, 0.35, fishBand) * (1.0 - smoothstep(0.35, 0.6, fishBand)) * 0.6);
          skin = mix(brycon, skin, fishHead * 0.35 * (1.0 - smoothstep(0.25, 0.33, fishX)));
          skin = mix(skin, vec3(0.040, 0.028, 0.024), exp(-pow((fishY - fishCleftY(fishX)) / 0.0030, 2.0)) * smoothstep(0.304, 0.322, fishX) * 0.85);
        #endif
        #ifdef FISH_SALMINUS
          // Dourado, Salminus brasiliensis: burnished gold from the bronze-olive back to a
          // pale-gold belly, every scale on the flank carrying a dark spot at its base so
          // that the rows read as fine dark lines running the length of the body.
          vec3 gold = mix(vec3(0.10, 0.07, 0.015), vec3(0.42, 0.28, 0.05), smoothstep(0.03, 0.28, fishBand));
          gold = mix(gold, vec3(0.86, 0.54, 0.09), smoothstep(0.24, 0.55, fishBand));
          gold = mix(gold, vec3(0.86, 0.76, 0.50), smoothstep(0.7, 0.98, fishBand));
          vec2 goldGrid = fishScaleGrid();
          vec2 goldCell = fract(goldGrid) - vec2(0.72, 0.5);
          float spot = (1.0 - smoothstep(0.10, 0.2, length(goldCell * vec2(1.0, 1.4))))
            * fishScaleMask() * smoothstep(0.12, 0.3, fishBand) * (1.0 - smoothstep(0.6, 0.8, fishBand));
          gold *= 1.0 + (fishHash(floor(goldGrid)) - 0.5) * 0.14 * fishScaleMask();
          gold = mix(gold, gold * vec3(0.18, 0.14, 0.1), spot * 0.8);
          // The black band on the caudal peduncle that runs on out along the tail.
          float stem = (1.0 - smoothstep(-0.29, -0.23, fishX)) * exp(-pow((fishBand - 0.5) / 0.12, 2.0));
          gold = mix(gold, vec3(0.02, 0.018, 0.012), stem * 0.85);
          skin = mix(gold, skin * vec3(1.25, 1.0, 0.55), fishHead * 0.3);
        #endif
        #ifdef FISH_SERPAE
          // Serpae tetra, Hyphessobrycon eques: rust-red flanks deepening to wine along the
          // back, a rosy belly, and the black comma behind the gill cover that gives the
          // species its name in the trade ("callistus", the beautiful).
          vec3 serpae = mix(vec3(0.085, 0.022, 0.016), vec3(0.40, 0.070, 0.032),
            smoothstep(0.04, 0.30, fishBand));
          serpae = mix(serpae, vec3(0.56, 0.15, 0.070), smoothstep(0.30, 0.62, fishBand));
          serpae = mix(serpae, vec3(0.66, 0.40, 0.33), smoothstep(0.74, 0.98, fishBand));
          vec2 serpaeGrid = fishScaleGrid();
          float serpaeRim = smoothstep(0.36, 0.5, length((fract(serpaeGrid) - 0.5) * vec2(0.85, 1.0)));
          serpae *= 1.0 + (fishHash(floor(serpaeGrid)) - 0.5) * 0.10 * fishScaleMask()
            - serpaeRim * 0.08 * fishScaleMask();
          float comma = exp(-pow((fishX - 0.152 + 0.02 * (fishBand - 0.36)) / 0.016, 2.0)
            - pow((fishBand - 0.36) / 0.13, 2.0));
          serpae = mix(serpae, vec3(0.012, 0.010, 0.010), comma * 0.92);
          skin = mix(serpae, skin * vec3(1.1, 0.75, 0.7), fishHead * 0.45);
        #endif
        ...
        #ifdef FISH_BRYCON
          // Orange-red lobes and anal fin, a dusky dorsal, and the black central band of
          // the tail carried out from the peduncle blotch.
          float dorsalFin = step(1.5, vFishPart) * (1.0 - step(2.5, vFishPart));
          vec3 fire = mix(vec3(0.62, 0.20, 0.035), vec3(0.72, 0.36, 0.06), span);
          diffuseColor.rgb = mix(membrane, fire, clamp(pigment * 1.1, 0.0, 1.0));
          diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.16, 0.16, 0.13), dorsalFin * 0.8);
          float band = caudal * exp(-pow((along - 0.5) / 0.16, 2.0)) * (1.0 - smoothstep(0.8, 1.0, span));
          diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.015, 0.015, 0.015), band * 0.92);
        #endif
        #ifdef FISH_SALMINUS
          {
            // Gold-orange dorsal and pelvics, a red anal fin, and the tail's red lobes
            // either side of the black band.
            float dorsalFin = step(1.5, vFishPart) * (1.0 - step(2.5, vFishPart));
            diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.66, 0.40, 0.07), dorsalFin * 0.85);
            diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.70, 0.14, 0.03), (1.0 - caudal) * (1.0 - dorsalFin) * (1.0 - pectoral) * 0.5);
          }
        #endif
        #ifdef FISH_SERPAE
          {
            // Black dorsal with a white tip; red anal fin edged black; red pelvics with
            // white tips; a rusty tail.
            float dorsalFin = step(1.5, vFishPart) * (1.0 - step(2.5, vFishPart));
            float analFin = step(2.5, vFishPart) * (1.0 - step(3.5, vFishPart));
            vec3 rust = vec3(0.50, 0.075, 0.030);
            diffuseColor.rgb = mix(membrane, rust, clamp(pigment * 1.15 + 0.15 * caudal, 0.0, 1.0));
            vec3 black = mix(vec3(0.012, 0.011, 0.012), vec3(0.62, 0.62, 0.58), smoothstep(0.72, 0.9, span));
            diffuseColor.rgb = mix(diffuseColor.rgb, black, dorsalFin);
            diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.015, 0.012, 0.012),
              analFin * smoothstep(0.62, 0.8, span) * (1.0 - smoothstep(0.9, 1.0, span)) * 0.9);
            pigment = max(pigment, dorsalFin);
          }
        #endif
      ...
      #ifdef FISH_BRYCON
        // A big characin's eye is a far smaller share of its head than a tetra's: the
        // outer orbit is skin, the iris a narrow gold ring round a small pupil.
        // The geometry's orbit has already been drawn in (flattenEyes), so the eye's own
        // radius is the shrunk one here.
        if (vFishPart > 6.5 && vFishPart < 10.5 && (vFishPart < 8.5 || vFishPart > 9.5)) {
          float orbit = fishOrbit() / ${glsl(EYE_SHRINK)};
          if (orbit > 0.8) diffuseColor.rgb = vec3(0.24, 0.23, 0.18);
          else if (orbit > 0.42) diffuseColor.rgb = mix(vec3(0.66, 0.5, 0.18), vec3(0.38, 0.28, 0.09), smoothstep(0.42, 0.8, orbit));
          else diffuseColor.rgb = vec3(0.006, 0.007, 0.008);
        }
      #endif
    `,
    )
    .replace(
      ...
      if (vFishPart < 0.5) {
        ...
        #ifdef FISH_SERPAE
          // A red fish: pigment over a much thinner reflector.
          metalnessFactor *= 0.4;
        #endif
        #ifdef FISH_SALMINUS
          // Gold is a reflector too: the flank mirrors the water in its own colour.
          metalnessFactor = max(metalnessFactor, 0.55 * smoothstep(0.15, 0.35, fishBand)
            * (1.0 - smoothstep(0.7, 0.9, fishBand)) * smoothstep(-0.29, -0.24, fishX));
        #endif
    ...
    .replace(
      ...
      if (vFishPart < 0.5) {
        ...
        #ifdef FISH_BRYCON
          // Scales a centimetre across catch the light one by one.
          relief *= 5.0;
        #endif
